...
const DEFAULT_INPUT = path.join(__dirname, "learning_items_ranked.json");
const DEFAULT_OUTPUT = path.join(
  __dirname,
  "learning_items_ranked_b1_unknown.json"
);
const DEFAULT_CACHE = path.join(
  __dirname,
  "learning_items_b1_unknown_probability_cache.json"
);
...
const DEFAULT_FALLBACK_BATCH_SIZE = 10;
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    switch (arg) {
      ...
      case "-b":
        ...
        index += 1;
        break;
      case "--fallback-batch-size":
        options.fallbackBatchSize = Number.parseInt(next, 10);
        index += 1;
        break;
      case "--max-tokens":
        ...
        index += 1;
        break;
      case "--average-with-existing":
        options.averageWithExisting = true;
        break;
      case "--rerun-all":
...
function showHelp() {
  console.log(`
Uso:
  node scripts/rank-learning-items-b1-lmstudio.js [opciones]

Opciones:
  -i, --input <ruta>             Archivo de entrada JSON (default: scripts/learning_items_ranked.json)
  -o, --output <ruta>            Archivo de salida JSON (default: scripts/learning_items_ranked_b1_unknown.json)
  ...
  -b, --batch-size <n>           Tamano del lote inicial (default: 15)
  --fallback-batch-size <n>      Tamano al bajar el lote tras el primer fallo (default: 10)
  --max-tokens <n>               Maximo de tokens de salida por solicitud (default: 768)
  --average-with-existing        Promedia el score nuevo con el existente y redondea a entero
  --rerun-all                    Reconsulta todos los items aunque ya existan en cache/salida
...
function getCandidateScoreValue(candidate) {
  ...
  const directKeys = [
    "b1UnknownProbability",
    "b1_unknown_probability",
    "unknownProbability",
    "unknown_probability",
    "unknownRate",
    "unknown_rate",
  ...
  for (const [key, value] of Object.entries(candidate)) {
    ...
    const looksLikeUnknownScore =
      (normalizedKey.includes("b1") && normalizedKey.includes("unknown")) ||
      normalizedKey.includes("unknown") ||
      normalizedKey.includes("unfamiliar") ||
      normalizedKey.includes("difficulty") ||
      normalizedKey.includes("score");

    if (looksLikeUnknownScore && value !== undefined) {
      return value;
    }
  }
...
function normalizeScoresResponse(parsed, batch) {
  ...
  if (missingIds.length > 0) {
    throw new Error(`Faltan scores para ids: ${missingIds.join(", ")}`);
  }
...
function buildPromptPayload(batch) {
  return batch.map((item) => ({
    id: item.id,
    label: item.label,
    conversationProbability: Number.isFinite(item.conversationProbability)
      ? item.conversationProbability
      : undefined,
      ...
      messages: [
        {
          ...
          content:
            ...
            "Score how likely it is that a typical Spanish-speaking learner with B1 English does NOT know the word in the target sense. " +
            "Return only valid JSON. Use an integer from 0 to 100, where 100 means it is highly likely a B1 learner does not know it, " +
            "70-89 means probably unknown, 40-69 means mixed or borderline for B1, 10-39 means probably already known by many B1 learners, " +
            "and 0-9 means almost certainly already known by a B1 learner.",
        },
        {
          role: "user",
          content: JSON.stringify({
            task: "Score vocabulary items by probability that a B1 English learner does not know them yet.",
            context:
              "The learners are Spanish speakers. The input file is already ranked by conversational usefulness, but this task is only about B1 familiarity.",
            rules: [
              "Keep the original ids.",
              "Return ONLY this JSON shape: {\"items\":[{\"id\":number,\"b1UnknownProbability\":number}]}",
              "Each item object must contain only id and b1UnknownProbability.",
              "Do not include labels, explanations, comments, markdown, or extra keys.",
              "Judge the specific sense implied by the examples and explanation, not rare alternate meanings.",
              "Common everyday words like basic verbs, colors, body parts, and elementary school vocabulary should usually be low.",
              "Less frequent, abstract, formal, idiomatic, or multi-word expressions can be much higher.",
            ],
...
function averageScoreMaps(baseScores, newScores) {
  const averaged = new Map();

  for (const [id, newScore] of newScores.entries()) {
    const previousScore = baseScores.get(id);

    if (Number.isFinite(previousScore)) {
      averaged.set(id, clampProbability(Math.round((previousScore + newScore) / 2)));
      continue;
    }

    averaged.set(id, clampProbability(newScore));
  }
...
async function scoreBatchAdaptive(batch, options) {
  try {
    ...
    console.warn(
      `Dividiendo sublote ${batch[0]?.id}-${batch[batch.length - 1]?.id} en ${leftBatch.length}+${rightBatch.length} por fallo persistente: ${error.message}`
    );
...
async function scoreBatchWithInitialFallback(batch, options) {
  if (batch.length <= options.fallbackBatchSize) {
    return requestBatchScoresWithRetry(batch, options);
  }
  ...
  try {
    ...
    console.warn(
      `Lote ${batch[0]?.id}-${batch[batch.length - 1]?.id} fallo en el primer intento. Bajando a sublotes de ${options.fallbackBatchSize}. Motivo: ${error.message}`
    );

    const smallerBatches = chunk(batch, options.fallbackBatchSize);
    const combined = new Map();

    for (const smallerBatch of smallerBatches) {
      mergeScoreMaps(
        combined,
        await scoreBatchAdaptive(smallerBatch, options)
      );
    }
...
function loadExistingScores(items, cachePath, outputPath) {
  ...
  for (const item of items) {
    if (Number.isFinite(item?.b1UnknownProbability)) {
      scoreMap.set(Number(item.id), clampProbability(item.b1UnknownProbability));
    }
  }
      ...
      if (Array.isArray(parsed)) {
        for (const item of parsed) {
          if (
            ...
            Number.isFinite(item?.b1UnknownProbability)
          ) {
            scoreMap.set(Number(item.id), clampProbability(item.b1UnknownProbability));
          }
        }
...
function enrichItems(items, scoreMap) {
...
async function main() {
  ...
  const fallbackBatchSize =
    Number.isFinite(args.fallbackBatchSize) && args.fallbackBatchSize > 0
      ? args.fallbackBatchSize
      : DEFAULT_FALLBACK_BATCH_SIZE;
  ...
  const shouldRerunAll = Boolean(args.rerunAll || args.averageWithExisting);
  const pendingItems = shouldRerunAll
    ? items.slice()
    : items.filter((item) => !scoreMap.has(Number(item.id)));
  ...
  console.log(`Lote inicial: ${batchSize}`);
  console.log(`Lote de fallback: ${fallbackBatchSize}`);
  console.log(`Promediar con existente: ${args.averageWithExisting ? "si" : "no"}`);
  console.log(`Reconsultar todos: ${shouldRerunAll ? "si" : "no"}`);
  ...
  for (let index = 0; index < batches.length; index += 1) {
    ...
    const freshBatchScores = await scoreBatchWithInitialFallback(batch, requestOptions);
    const finalBatchScores = args.averageWithExisting
      ? averageScoreMaps(scoreMap, freshBatchScores)
      : freshBatchScores;

    for (const [id, score] of finalBatchScores.entries()) {
  ...
  const outputItems = enrichItems(items, scoreMap);
  writeJsonFile(outputPath, outputItems);
