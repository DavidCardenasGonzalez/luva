...
const CARD_STATUS_COPY: Record<CardProgressStatus, string> = {
  todo: 'Por aprender',
...
export function WelcomePage() {
  ...
  useEffect(() => {
    ...
    const loadDashboard = async () => {
      try {
        ...
        const message =
          loadError instanceof Error ? loadError.message : 'No pudimos cargar tu dashboard.'
        setError(message)
      } finally {
        if (!cancelled) {
          setLoading(false)
  ...
  const dashboardSummary = useMemo(() => {
    ...
    const activity = buildRecentActivity(progress, storyRows)
      ...
      <main className="dashboard-shell">
        <section className="dashboard-hero-card">
          <div className="dashboard-hero-copy">
            ...
            <p className="lede">
              Aquí ves el progreso sincronizado entre el app y la web usando el mismo usuario de
              Cognito.
            </p>
              ...
              {(['learned', 'learning', 'todo'] as CardProgressStatus[]).map((status) => {
                ...
                return (
                  <div key={status} className={`dashboard-card-status dashboard-card-status-${status}`}>
                    <span>{CARD_STATUS_COPY[status]}</span>
                    <strong>{count}</strong>
...
function buildRecentActivity(
  ...
  for (const [cardId, entry] of Object.entries(progress.cards.items || {})) {
    activity.push({
      key: `card-${cardId}`,
      label: `Card ${cardId}`,
      detail: `Estado actual: ${CARD_STATUS_COPY[entry.status]}.`,
      at: entry.updatedAt,
    })
  }
